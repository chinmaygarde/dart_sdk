...
bool OSThread::GetCurrentStackBounds(uword* lower, uword* upper) {
  int asfd = TEMP_FAILURE_RETRY(open("/proc/self/as", O_RDONLY));
  if (asfd == -1) {
    return false;
  }

  procfs_status stat_buf = {};

  stat_buf.tid = pthread_self();

  int rc = devctl(asfd, DCMD_PROC_TIDSTATUS, &stat_buf, sizeof(stat_buf), NULL);
  close(asfd);
  if (rc != EOK) {
    return false;
  }
  ...
  const uint8_t* stack_base =
      reinterpret_cast<const uint8_t*>(stat_buf.stkbase);
  const auto stack_size = stat_buf.stksize;

  if (lower) {
    *lower = reinterpret_cast<uword>(stack_base);
  }
  if (upper) {
    *upper = reinterpret_cast<uword>(stack_base + stack_size);
  }
  return true;
}
